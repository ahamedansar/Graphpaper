      ...
      {orders.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {orders.map(order => {
            ...
            return (
              <div key={order._id} style={{ backgroundColor: '#fff', borderRadius: '16px', padding: '20px 24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', border: '1px solid #F5F5F5' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '10px' }}>
                    <span style={{ fontWeight: '800', fontSize: '18px', color: '#16a34a' }}>₹{order.totalPrice.toFixed(2)}</span>
                ...
                <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #F5F5F5', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Link to={`/orders/${order._id}`} style={{ display: 'flex', alignItems: 'center', gap: '6px', textDecoration: 'none', color: '#E50010', fontWeight: '700', fontSize: '13px' }}>
