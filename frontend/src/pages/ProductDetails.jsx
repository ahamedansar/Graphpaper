    ...
    <div className="py-4">
      ...
      <div className="row g-5">
        ...
        <div className="col-lg-6 d-flex flex-column py-4">
          ...
          <a
            href={`https://wa.me/917012056376?text=Hi%20Graphpaper!%20I%20am%20interested%20in%20bulk%20order%20for%20*${encodeURIComponent(product.name)}*%20(₹${product.price}/unit).%20Please%20share%20more%20details.`}
            target="_blank"
            rel="noopener noreferrer"
            className="btn btn-lg w-100 py-3 mt-3 d-inline-flex justify-content-center align-items-center gap-2 fw-bold"
            style={{ backgroundColor: '#25D366', color: '#fff', borderRadius: 0, letterSpacing: '1px' }}
          >
          ...
          {/* Bulk Discount Tiers */}
          <div className="mt-4 p-4" style={{ backgroundColor: '#F0FFF4', border: '1px solid #BBF7D0', borderRadius: '12px' }}>
            <div className="fw-bold mb-3" style={{ fontSize: '13px', textTransform: 'uppercase', letterSpacing: '1px', color: '#15803d' }}>🏷️ Bulk Discount Tiers</div>
            <table className="w-100" style={{ fontSize: '13px', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #BBF7D0' }}>
                  <th style={{ padding: '6px 0', color: '#166534', fontWeight: 700 }}>Qty (per size)</th>
                  <th style={{ padding: '6px 0', color: '#166534', fontWeight: 700, textAlign: 'right' }}>Price/Unit</th>
                  <th style={{ padding: '6px 0', color: '#166534', fontWeight: 700, textAlign: 'right' }}>Savings</th>
                </tr>
              </thead>
              <tbody>
                {[
                  { range: '10 – 49 units', discount: 0 },
                  { range: '50 – 99 units', discount: 5 },
                  { range: '100 – 199 units', discount: 10 },
                  { range: '200+ units', discount: 15 },
                ].map((tier, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #DCFCE7', backgroundColor: totalQty >= [10, 50, 100, 200][i] && totalQty < [50, 100, 200, 99999][i] ? '#DCFCE7' : 'transparent' }}>
                    <td style={{ padding: '8px 0', fontWeight: 600 }}>{tier.range}</td>
                    <td style={{ padding: '8px 0', textAlign: 'right', fontWeight: 700, color: '#15803d' }}>₹{(product.price * (1 - tier.discount / 100)).toFixed(2)}</td>
                    <td style={{ padding: '8px 0', textAlign: 'right', color: tier.discount > 0 ? '#16a34a' : '#888', fontWeight: 700 }}>{tier.discount > 0 ? `${tier.discount}% OFF` : 'Base'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
